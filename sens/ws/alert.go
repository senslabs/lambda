...
func ListAlertRules(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := getAuthSubject(r); err != nil {
		...
		params := httpclient.HttpParams{"limit": {"null"}, "and": {"UserId^" + userId}}
		url := fmt.Sprintf("%s/api/alert-rules/find", GetDatastoreUrl())
		code, data, err := httpclient.GetR(url, params, nil)
		defer r.Body.Close()
		logger.Debugf("%d, %s", code, data)
		if err != nil {
...
func ListAlertEscalations(w http.ResponseWriter, r *http.Request) {
	...
	if _, err := getAuthSubject(r); err != nil {
		...
		userId := r.URL.Query().Get("userId")
		params := httpclient.HttpParams{"limit": {"null"}, "and": {"UserId^" + userId}}
		url := fmt.Sprintf("%s/api/alert-escalations/find", GetDatastoreUrl())
		code, data, err := httpclient.GetR(url, params, nil)
		defer r.Body.Close()
		logger.Debugf("%d, %s", code, data)
		if err != nil {
